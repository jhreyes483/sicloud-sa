...
//Metodos de facturacion interna
...
public function facturar($a , $tipo = 1){
    ...
    public function verFactura($id){
        $aFactura = $this->objMod->verFactura($id);
        if( count($aFactura) == 0){
          return  ['response_status'=> 'error', 'response_msg' => 'No existe factura' ];
        }else{
            $aProd = $this->objMod->consProductosFactura($id);
            return['response_status' => 'OK', 'response_msg' => [
                    $aFactura,
                    $aProd
                ]
